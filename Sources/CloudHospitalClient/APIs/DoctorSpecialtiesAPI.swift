...
open class DoctorSpecialtiesAPI {
    ...
    open class func apiV1DoctorsDoctorIdSpecialtySpecialtyIdPost(doctorId: UUID, specialtyId: UUID, apiResponseQueue: DispatchQueue = CloudHospitalClient.apiResponseQueue) -> AnyPublisher<DoctorSpecialtyViewModel, Error> {
    ...
    open class func apiV1DoctorsDoctorIdSpecialtySpecialtyIdPostWithRequestBuilder(doctorId: UUID, specialtyId: UUID) -> RequestBuilder<DoctorSpecialtyViewModel> {
        var localVariablePath = "/api/v1/doctors/{doctorId}/specialty/{specialtyId}"
        ...
        let localVariableRequestBuilder: RequestBuilder<DoctorSpecialtyViewModel>.Type = CloudHospitalClient.requestBuilderFactory.getBuilder()
    ...
    #if canImport(Combine)
    @available(OSX 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
    open class func apiV1DoctorsHospitalIdDoctorsDoctorIdDelete(doctorId: UUID, specailtyId: UUID, hospitalId: String, apiResponseQueue: DispatchQueue = CloudHospitalClient.apiResponseQueue) -> AnyPublisher<Bool, Error> {
    ...
    open class func apiV1DoctorsHospitalIdDoctorsDoctorIdDeleteWithRequestBuilder(doctorId: UUID, specailtyId: UUID, hospitalId: String) -> RequestBuilder<Bool> {
        var localVariablePath = "/api/v1/doctors/{hospitalId}/doctors/{doctorId}"
        let doctorIdPreEscape = "\(APIHelper.mapValueToPathItem(doctorId))"
        let doctorIdPostEscape = doctorIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        localVariablePath = localVariablePath.replacingOccurrences(of: "{doctorId}", with: doctorIdPostEscape, options: .literal, range: nil)
        ...
        localVariablePath = localVariablePath.replacingOccurrences(of: "{specailtyId}", with: specailtyIdPostEscape, options: .literal, range: nil)
